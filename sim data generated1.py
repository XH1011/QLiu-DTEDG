...
# ================== 参数定义 ==================
# ------ 齿轮几何参数 ------
r_1 = 0.0415  # 【微调】直齿轮分度圆半径 (m)
r_2 = 0.0938  # 【微调】面齿轮分度圆半径 (m)
m = 0.0045    # 模数 (m)
h_a = 1.0 * m
h_f = 1.25 * m
...
k_x1 = k_z1 = k_x2 = k_z2 = 1.18e6  # 【微调】支撑刚度 (N/m)

# ------ 阻尼参数 ------
zeta = 0.012   # 【微调】
c_x1 = c_z1 = 1.02   # 【微调】阻尼 (N·s/m)
c_x2 = c_z2 = 1.02   # 【微调】

# ------ 负载参数 ------
T_load = 998     # 【微调】阻力矩 (N·m)
F = T_load / r_2

# ------ 间隙和误差激励 ------
b0 = 0.21e-3    # 【微调】齿侧间隙 (m)
Delta_lambda = 0.105e-3  # 【微调】
e0 = 98e-6      # 【微调】误差常量 (m)
er = 195e-6     # 【微调】
Omega_h = 105 * 2 * np.pi
c_m = 10.8      # 【微调】
...
class EnhancedStiffnessGenerator:
    def __init__(self, normal_data, fault_data, fs, theory_points):
        self.fs = fs
        self.cycle_time = 1 / 105  # 啮合周期（与转速对应）
        self.transition_ratio = 0.12  # 【微调】过渡系数

        self.normal_data = normal_data
        self.fault_data = fault_data

        t_normal = np.linspace(0, self.cycle_time, len(self.normal_data))
        self.normal_interp = CubicSpline(t_normal, self.normal_data,
                                         bc_type=((2, 0.0), (2, 0.0)))

        t_fault = np.linspace(0, self.cycle_time, len(self.fault_data))
        self.fault_interp = CubicSpline(t_fault, self.fault_data,
                                        bc_type=((2, 0.0), (2, 0.0)))
    ...
    def _get_raw_stiffness(self, t, cycle_num):
        ...
        if (cycle_num % 48) == 47:
            return self.fault_interp(t_clipped)
        else:
...
y0 = [1.2e-6, 0, 1.2e-6, 0, X2_static, 0, Z2_static, 0, lambda_initial, 0]  # 【微调】
...
output_path = os.path.join(output_dir, '裂纹振动加速度数据519.xlsx')
...
mesh_freq = 420
plt.axvline(mesh_freq, color='r', linestyle='--', label=f'啮合频率{mesh_freq}Hz')

fault_freq = 8.75
plt.axvline(fault_freq, color='orange', linestyle='--',
           label=f'故障特征频率{fault_freq}Hz')

for n in range(1, 6):
    upper = mesh_freq + n * fault_freq
    lower = mesh_freq - n * fault_freq
    plt.axvline(upper, color='grey', linestyle=':', alpha=0.5)
    plt.axvline(lower, color='grey', linestyle=':', alpha=0.5)
