...
def train_test_split_(data_pd, test_size=0.2, label_set=None):
...
class dataset(Dataset):
    ...
    def __getitem__(self, item):
        seq = self.seq_data[item]  # numpy.ndarray, shape=(128,)
        seq = self.transforms(seq)  # 应返回 torch.Tensor
        seq = torch.from_numpy(seq).unsqueeze(0)  # 先转Tensor再unsqueeze
        if self.test:
            return seq  # torch.Size([1, 128])
        else:
            return seq, self.actual_labels[item]
