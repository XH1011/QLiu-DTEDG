...
# ---------- 数据加载与处理 ----------
path = f'./features/Cross/Cross_S1/cross_{gear_type}-.pkl'
with open(path, 'rb') as f:
    raw_data = pickle.load(f)

# 兼容 dict, (test,train), tf.Tensor, np.ndarray
if isinstance(raw_data, dict):
    key = next(iter(raw_data))
    test, train = raw_data[key]
elif isinstance(raw_data, (list, tuple)) and len(raw_data) == 2:
    test, train = raw_data
elif isinstance(raw_data, tf.Tensor):
    arr = raw_data.numpy()
    test = np.empty((0, arr.shape[1]), dtype=np.float32)
    train = arr
elif isinstance(raw_data, np.ndarray):
    test = np.empty((0, raw_data.shape[1]), dtype=np.float32)
    train = raw_data
else:
    raise ValueError(f"❌ 不支持的数据格式：{type(raw_data)}")

# 合并并转换为 float32
data_train = np.vstack([test, train]).astype(np.float32)
...
for epoch in range(num_epochs):
    ...
    if epoch % 100 == 0:
        save_dir = f'./data_face_gear/Imbalance_{imbalanced_sample}/model{gear_type}/'
...
np.savetxt(f'./data_face_gear/Imbalance_{imbalanced_sample}/ddpm_loss_{gear_type}.txt',
           np.array(loss_list))
...
# 保存生成结果
out_path = f'./features/Imbalance_{imbalanced_sample}/Gen_face_gear_{gear_type}.pkl'
