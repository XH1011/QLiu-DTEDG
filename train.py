...
def creat_file(args): #1. 模型文件保存准备/2. 日志系统初始化/3. 实验元数据管理
...
if __name__ == '__main__':
    os.environ['NUMEXPR_MAX_THREADS'] = '0'
    # 解析命令行参数，设置默认值，重要参数包括：模型名称、源域/目标域、训练模式、学习率、batch size等
    args = parse_args()
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    if str(device) == 'cpu':
        logging.warning('CUDA is not available, using CPU instead. Performance may be degraded.')
    ...
    #源域处理：将逗号分隔的源域名称转换为列表（opt厘米的args.source变成了遍历的args）
    #比如输入args.source = "CWRU_0, CWRU_1 ,CWRU_2" ，输出是 args.source_name = ['CWRU_0', 'CWRU_1', 'CWRU_2']
    args.source_name = [x.strip() for x in list(args.source.split(','))]
    if '' in args.source_name:
        args.source_name.remove('')

    #检查训练模式与源域数量的匹配性，是用于验证训练模式与源域数量的匹配性的逻辑
    ...
    # creating directory 从数据集目录读取故障类型、计算类别数量、记录日志
    ...
    ##作用：创建两个空列表，用于存储：
    # faults：每个域的故障类型名称列表（如 ['normal', 'inner', 'ball']）
    # num_classes：每个域的故障类别数量（如 3）
    args.faults, args.num_classes = [], []


    for name in args.source_name + [args.target]:
        faults, num_classes = get_fault(name, args)
        #args.faults = [['normal', 'inner_007', 'ball_014'],  # CWRU_0
        # ['normal', 'outer_021']/# CWRU_1
        # ['normal', 'inner_007']# CWRU_2
        # args.num_classes = [3, 2, 2]
        args.faults.append(faults)
        args.num_classes.append(num_classes)

        #Source CWRU_0 detected 3 classes: ['normal', 'inner_007', 'ball_014']
        #Source CWRU_1 detected 2 classes: ['normal', 'outer_021']
        #Target CWRU_2 detected 2 classes: ['normal', 'inner_007']
    ...
    all_faults = set() # 初始化空集合
    for faults in args.faults: # 遍历所有域（源域+目标域
        for item in faults: # 遍历当前域的故障列表
            all_faults.add(item) # 添加故障名称（自动去重）
